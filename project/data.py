...
class FaceGANDataset(data.Dataset):
    """Define dataset."""

    def __init__(self, root, transforms=get_transform()):
        """Init dataset."""
        super(FaceGANDataset, self).__init__()
...
def train_data(bs):
    ...
    train_ds = FaceGANDataset(train_dataset_rootdir, get_transform(train=True))
    print(train_ds)
...
def test_data(bs):
    ...
    test_ds = FaceGANDataset(test_dataset_rootdir, get_transform(train=False))
...
def FaceGANDatasetTest():
    """Test dataset ..."""

    ds = FaceGANDataset(train_dataset_rootdir)
    print(ds)
    # src, tgt = ds[0]
